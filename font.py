...
def resolve_texture_file(file_ref, default_namespace="minecraft"):
    if ":" in file_ref:
    ...
    # Candidate paths
    candidates = [
        os.path.join("pack", "assets", namespace, "textures", subpath),
        os.path.join("pack", "assets", namespace, "textures", "font", subpath),
        os.path.join("pack", "assets", namespace, "font", subpath),
        os.path.join("pack", "assets", namespace, subpath),
        os.path.join("pack", "assets", "minecraft", "textures", subpath),
        os.path.join("pack", "assets", "minecraft", "textures", "font", subpath),
    ]
...
def process_font_files():
    font_files = glob.glob("pack/assets/**/font/*.json", recursive=True)
    if not font_files:
        print("[FONT] No font definition files found in pack/assets/**/font/*.json")
        return

    print(f"[FONT] Found font definition files: {font_files}")
    ...
    for font_path in font_files:
        try:
            ...
            # pack/assets/<namespace>/font/<file>.json
    ...
    # Generate Bedrock glyph sheets
    out_dir = "staging/target/rp/font"
    os.makedirs(out_dir, exist_ok=True)
    generated_count = 0

    for page_hex, glyph_entries in pages.items():
        try:
